...
feature {CAPTURE_REPLAY} -- CAPTURE-ADDONS

-- handling of capture/replay

	display_poi is
			-- 
	require
		-- trace_file /= void and then trace_file.is_open_write
	do
		trace_file.putstring ("%N ++++++++++++++++++++++++++++ %N")
		trace_file.putstring (" Info from WEL-WINDOW already created (array : reg_windows) ")
		trace_file.putstring ("%N ----- Type  : ")
		trace_file.putstring (generating_type)
		trace_file.putstring ("%N ----- parent POI  : ")
		if parent /= void then
			trace_file.putstring (parent.generating_type)
		else
			trace_file.putstring ("%N ----- This window has no parent ")
		end
		
		trace_file.putstring ("%N ----- X  : ")
		trace_file.putint (x)
		trace_file.putstring ("%N ----- Y : ")
		trace_file.putint (y)	
		trace_file.putstring ("%N ----- Width  : ")
		trace_file.putint (width)
		trace_file.putstring ("%N ----- Height : ")
		trace_file.putint (height)
		trace_file.putstring ("%N ++++++++++++++++++++++++++++ %N")
	end
	
	match_poi (p : CAPTURE_REPLAY_POI): BOOLEAN is
		-- poi for capture-replay purpose
		require
			p /= void
		local
--			tmp: WEL_WINDOW
		do
--			if not p.has_no_parent then
--				tmp := p.parent_identifier.object
--				check
--					tmp /= void
--				end
--			end
			if parent /= void then
				Result := p.type.is_equal (generating_type) and then
				   p.parent_type.is_equal (parent.generating_type) and then
				   p.parent_x = parent.x and then p.parent_y = parent.y and then
				   p.parent_width = parent.width and then p.parent_height = parent.height and then
	--			   tmp = parent and then
				   p.top_x = x and then p.top_y = y and then
				   p.width = width and then p.height = height 
			else
				Result :=  p.type.is_equal (generating_type) and then
				   p.parent_type = void and then
				   p.parent_x = 0 and then p.parent_y = 0 and then
				   p.parent_width = 0 and then p.parent_height = 0 and then
				   p.top_x = x and then p.top_y = y and then
				   p.width = width and then p.height = height 
			end
		end
