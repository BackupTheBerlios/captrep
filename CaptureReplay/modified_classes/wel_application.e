...
inherit	
	CAPTURE_REPLAY_HANDLING -- CAPTURE-ADDON
...
feature {NONE} -- Implementation
	...
	message_loop is
		...
		local
			...
			dlg: POINTER
		do
io.putstring ("ENTRER DANS MESSAGE_LOOP de WEL_APPLICATION%N")
			...
			if accel /= Void then
				...
				loop
					...
					if is_replay and then not is_last_captured then
						...
						msg.peek_all
						if is_capture then
io.putstring ("On veut capturer un événement%N")
							capture_event (msg)
						end
					end
					
					-- msg.peek_all (removed with CAPTURE-ADDON)
