...
*                necessaire à l'application PataCara.
...
public class BaseRegistreWin
{
     ...
     */
    private static boolean installCleRepInstall ()
    {
    ...
    private static boolean removeCleRepInstall ()
    {
      return (ATTR_SUPPRIMER ==
              supprimerAttribut (TOP_CLE_DEST, SUB_CLE_DEST + "\\" + NOM_CLE,
                                 NOM_ATTR_REP_INSTALL));
    }/* removeCleRepInstall */
    ...
    private static boolean createAttrRepInstall ()
    {
        ...
        int reponse;
        if (ERREUR == (reponse = creerOuOuvrirCle(keyPata, SUB_CLE_DEST + "\\" + NOM_CLE)))
        {
          System.err.println("Impossible de créer la clé" + SUB_CLE_DEST + "\\" + NOM_CLE);
          return false;
        }
        else if (CLE_EXISTE == reponse)
        {
          System.out.println("La clé '" + SUB_CLE_DEST + "\\" + NOM_CLE +"' existait déjà");
        ...
        String repInstall = getValueAttr(keyPata, SUB_CLE_DEST + "\\" + NOM_CLE,
                                         NOM_ATTR_REP_INSTALL);
        if (null == repInstall)
        ...
        {//Attribut présent, on le supprime
          ...
          if (ERREUR ==
              (reponse = supprimerAttribut (keyPata, SUB_CLE_DEST + "\\" + NOM_CLE,
                                            NOM_ATTR_REP_INSTALL)))
        ...
        return setValueAttr(keyPata, SUB_CLE_DEST + "\\" + NOM_CLE, NOM_ATTR_REP_INSTALL,
                     System.getProperty("user.dir"));
